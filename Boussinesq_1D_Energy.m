...
CFL = 2; dt =  1; %CFL*dx/cV; 
% NOTE - uses a different dt for COMP6
...
[uEx6, utEx6] = uExact(tCOMP6(end), xGrid, CaseNo); 
...
normOut = [m, dx, dt, normCalc]; 


figure; 
plot(xGrid, uCOMP4(1:m+2, 1), '-', 'LineWidth', 1);
hold on; plot(xGrid, uCOMP4(1:m+2, end), '-r', 'LineWidth', 2);
hold on; plot(xGrid, uCOMP6(1:m+2, end-1), '-.c', 'LineWidth', 2);
hold on; plot(xGrid, uRRK(1:m+2, end), '--.b', 'LineWidth', 1.2);
hold on; plot(xGrid, uEx, '.k', 'LineWidth', 1.1);
title('u vs x, 1D Boussinesq Eq'); 
legend('initial', 'MIM4-COMP4', 'MIM6-COMP6', 'MIM4-RK4', 'Exact', 'location', 'best'); 
xlabel('x'); ylabel('u(t,x)'); set(gca,'FontSize',10)


E0ref = eRRK(2); E0refCOMP = ECOMP4(2); 
E0refCOMP6 = ECOMP6(2); 
pn1 = 2; pn2 = 1; figure; 
plot(tCOMP4(pn1:pn2:end), abs(ECOMP4(pn1:pn2:end)/E0refCOMP), '-.k', 'LineWidth', 1.2);  %
hold on; plot(tCOMP6(pn1:5:end), abs(ECOMP6(pn1:5:end)/E0refCOMP6), 'sr', 'LineWidth', 1.2); 
hold on; plot(tRRK(pn1:pn2:end), abs(eRRK(pn1:pn2:end) / E0ref), '--b', 'LineWidth', 1.2); %
title('Energy En/E0 vs time, 1D Boussinesq Eq'); %set(gca, 'yscale', 'log');
