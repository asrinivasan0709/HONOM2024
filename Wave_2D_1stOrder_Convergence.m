...
for i = 1:size(mVec,1)
    fprintf(' ... Executing Loop %d of %d ... \n', i, size(mVec,1)); 
    Mult = mVec(i); 
    m = Mult*64; xmin = -15; xmax = 15; 
    n = Mult*64; ymin = -15; ymax = 15; 
    dx = (xmax-xmin)/m; dy = (ymax-ymin)/n;
    tEnd = 1; cV = 1; 
    CFL = 0.5; dt = CFL*dx/cV; %0.01; % 
    ...
    uEx = uExact(tRRK(end), X, Y, CaseNo); uEx = uEx(:); 
    uRRKOut = uRRK(1:(m+2)*(n+2), end); 
    uCOMP4Out = uCOMP4(1:(m+2)*(n+2), end); 
    uCOMP6Out = uCOMP6(1:(m+2)*(n+2), end); 
    
    normOut(i, :) = [m, dx, dt, norm(uEx - uRRKOut, 'inf'), ...
...
% title('Energy En/E0 vs time, Wave Eq 2D'); %set(gca, 'yscale', 'log');
...
uCOMP4(:,1) = [U; V]; % initial value
    ...
    D = div2D(4, m, dx, n, dy); G = grad2D(4, m, dx, n, dy);
...
t = dt; 
    ...
    %
        ...
        ECOMP4(iCount,1) = 0.5*dx*dy*(p5'*p5 + q5'*q5 ); % - E0; 
        uCOMP4(:,iCount) = uNew;   
...
uCOMP4(:,1) = [U; V]; % initial value
    ...
    D = div2D(6, m, dx, n, dy); G = grad2D(6, m, dx, n, dy);
...
t = dt; 
    ...
    %
        ...
        ECOMP4(iCount,1) = 0.5*dx*dy*(p7'*p7 + q7'*q7 ); %- E0; 
        uCOMP4(:,iCount) = uNew;   
        U = uCOMP4(1:(m+2)*(n+2), iCount);
        V = uCOMP4((m+2)*(n+2)+1:end, iCount);
...
fprintf('Executing RK4 ... \n'); 
    ...
    while t <= tEnd
       ...
       eRRK(iCount,1) = 0.5*dx*dy*(Uin'*Uin + ...
            Vin'*Vin ); %- E0;  
...
function uOut = uExact(t, X, Y, CaseNo)

switch CaseNo
    case 1    
        A = 1; sigma = 0.5; mu = 0;         
        uOut = A*exp(-1/(sigma^2)*((Y-mu/2 - t).^2 + (X-mu/2 - t).^2));
