...
hold on; plot(xGrid, uCOMP6(1:m+2, end), 'sr', 'LineWidth', 0.5);
hold on; plot(xGrid, uRRK(1:m+2, end), '--b', 'LineWidth', 1.2);
hold on; plot(xGrid, uEx, '--r', 'LineWidth', 0.9);
title('u vs x, 1D Sine Gordon Eq'); 
...
hold on; plot(tCOMP6(pn1:pn2:end), abs(ECOMP6(pn1:pn2:end)), 'sr', 'LineWidth', 1.2); 
hold on; plot(tRRK(pn1:pn2:end), abs(eRRK(pn1:pn2:end)), '--b', 'LineWidth', 1.2); 
title('Energy En-E0 vs time, 1D Sine Gordon Eq'); set(gca, 'yscale', 'log');
